...
//Code creates an iterator over the items in the vector v1 by calling the iter method defined on Vec<T>. This code by itself doesn’t do anything useful.
...
//Before we used iterators with for loops to execute some code on each item.

//The example separates the creation of the iterator from the use of the iterator in the for loop.
...
//you would likely write this same functionality by starting a variable at index 0, using that variable to index into the vector to get a value, 
...
//Example of calling the iterator adaptor method map, which takes a closure to call on each item to produce a new iterator. 
//The closure here creates a new iterator in which each item from the vector has been incremented by 1. However, this code produces a warning:
...
//The closure we’ve specified never gets called. The warning reminds us why: iterator adaptors are lazy, and we need to consume the iterator here.
...
//In Listing 13-18, we collect the results of iterating over the iterator that’s returned from the call to map into a vector. 
