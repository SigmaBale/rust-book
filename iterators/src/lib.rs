...
//We’ll talk about associated types in depth later.
...
//and this Item type is used in the return type of the next method. In other words, the Item type will be the type returned from the iterator.
...
//We use filter with a closure that captures the shoe_size variable from its environment to iterate over a collection of Shoe struct instances. 
...
//You can create iterators from the other collection types in the standard library, such as hash map.
...
//Once you’ve done that, you can use all other methods that have default implementations provided by the Iterator trait!

//To demonstrate, let’s create an iterator that will only ever count from 1 to 5. First, we’ll create a struct to hold some values. 
//Then we’ll make this struct into an iterator by implementing the Iterator trait and using the values in that implementation.

//Definition of the Counter struct and an associated new function to create instances of Counter:
pub struct Counter {
    count: u32,
}
...
//The Counter struct has one field named count. This field holds a u32 value that will keep track of where we are in the process of iterating from 1 to 5. 
//The count field is private because we want the implementation of Counter to manage its value.
//The new function enforces the behavior of always starting new instances with a value of 0 in the count field.
...
//We want our iterator to add 1 to the current state, so we initialized count to 0 so it would return 1 first. 
//If the value of count is less than 5, next will increment count and return the current value wrapped in Some. 
//Once count is 5, our iterator will stop incrementing count and always return None.


//Using Our Counter Iterator’s next Method

//Once we’ve implemented the Iterator trait, we have an iterator!
//Test demonstrating that we can use the iterator functionality of our Counter struct by calling the next method on it directly, 
//just as we did with the iterator created from a vector.
...
//verifying that we have implemented the behavior we want this iterator to have: returning the values from 1 to 5.
...
//multiply each pair together, keep only those results that are divisible by 3, and add all the resulting values together, we could do so!
...
//and the standard library provides default implementations for other methods that call next.
