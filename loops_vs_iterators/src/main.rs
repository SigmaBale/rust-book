...
//the version of the query function with an explicit for loop or the version with iterators.

//We ran a benchmark by loading the entire contents of The Adventures of Sherlock Holmes by Sir Arthur Conan Doyle into a String and 
//looking for the word the in the contents. Here are the results of the benchmark on the version of search using the for loop and the version using iterators:

//test bench_search_for  ... bench:  19,620,300 ns/iter (+/- 915,700) - loops
...
//point is not to prove that the two versions are equivalent but to get a general sense of how these two implementations compare performance-wise.
...
//This code uses an iterator chain to do some math on three variables in scope: a buffer slice of data, 
//an array of 12 coefficients, and an amount by which to shift data in qlp_shift. 

// We’ve declared the variables within this example but not given them any values; although this code doesn’t have much meaning outside of its context, 
//it’s still a concise, real-world example of how Rust translates high-level ideas to low-level code.
fn main() {
    /*let buffer: &mut [i32];
...
//To calculate the value of prediction, this code iterates through each of the 12 values in coefficients and uses the zip method 
//to pair the coefficient values with the previous 12 values in buffer.
...
// >> this is bitwise operator, usage: x >> y, x takes number and y takes number, it shifts bits to the right side depending on y value, if y = 2 bits shift 2 spaces to right.
// It is same as dividing x by 2^y, example: z = 1000 >> 2, z = 250.
...
//All of the coefficients get stored in registers, which means accessing the values is very fast. There are no bounds checks on the array access at runtime. (IMPORTANT)
